...
contract AaveV3YieldManager is BaseYieldManager {
    ...
    function _wrapToken(address token, uint256 amount) internal {
        ...
        WrappedRebasingERC20(tokenToWrapper[token]).depositFor(address(this), amount);
    }
    ...
    function _getATokenAddress(address underlying) internal returns (address) {
        DataTypes.ReserveData memory reserveData = aavePoolProxy.getReserveData(underlying);
    ...
    function collectYield(
        ...
        uint256 amount = WrappedRebasingERC20(wrapper).recoverUnderlying();
        if (amount == 0) {
