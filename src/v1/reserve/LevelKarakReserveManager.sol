...
contract KarakReserveManager is LevelBaseReserveManager, IKarakReserveManager {
    ...
    function depositToKarak(address vault, uint256 amount)
        external
        onlyRole(MANAGER_AGENT_ROLE)
        returns (uint256 shares)
    {
        SafeERC20.forceApprove(IERC20(IKarakVault.IVault(vault).asset()), vault, amount);
    ...
    {
        withdrawalKey = IKarakVault.IVault(vault).startRedeem(shares, address(this));
