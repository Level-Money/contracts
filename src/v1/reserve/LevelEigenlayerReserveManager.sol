...
contract EigenlayerReserveManager is LevelBaseReserveManager {
        ...
        whenNotPaused
    {
        ...
        IStrategyManager(strategyManager).depositIntoStrategy(IStrategy(strategy), tokenContract, amount);
    }
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            IStrategyManager(strategyManager).depositIntoStrategy(strategies[i], token, balance);
        }
    }
        ...
        returns (bytes32[] memory)
    {
        ...
        IDelegationManager.QueuedWithdrawalParams memory withdrawalParam = IDelegationManager.QueuedWithdrawalParams({
        ...
        return IDelegationManager(delegationManager).queueWithdrawals(withdrawalParams);
    }
    ...
    function completeQueuedWithdrawal(
        ...
        if (tokens.length != strategies.length || tokens.length != shares.length || strategies.length != shares.length)
        {
        ...
        IDelegationManager(delegationManager).completeQueuedWithdrawal(
            withdrawal, tokens, 0, /* middleware index is currently a no-op */ true /* receive as tokens*/
        );
    }
