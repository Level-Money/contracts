 ...
 */
abstract contract LevelBaseReserveManager is ILevelBaseReserveManager, SingleAdminAccessControl, Pausable {
    ...
    function depositForYield(address token, uint256 amount) external onlyRole(MANAGER_AGENT_ROLE) whenNotPaused {
        ...
        emit DepositedToYieldManager(token, address(yieldManager[token]), amount);
    }
     ...
     * @dev only callable by manager agent
     */
    function withdrawFromYieldManager(address token, uint256 amount)
        external
        onlyRole(MANAGER_AGENT_ROLE)
        whenNotPaused
    {
        yieldManager[token].withdraw(token, amount);
        emit WithdrawnFromYieldManager(token, address(yieldManager[token]), amount);
    }
    ...
    function _mintlvlUSD(address collateral, uint256 collateralAmount) internal whenNotPaused {
        IERC20(collateral).forceApprove(address(levelMinting), collateralAmount);
        ...
        lvlUSDAmount -= lvlUSDAmount.mulDivDown(maxSlippageThresholdBasisPoints, MAX_BASIS_POINTS);
    ...
    function rewardStakedlvlUSD(address token) external onlyRole(MANAGER_AGENT_ROLE) whenNotPaused {
        uint256 amount = yieldManager[token].collectYield(token);
        (, uint256 collateralAmount) = _takeRake(token, amount);
        if (collateralAmount == 0) {
            revert InvalidAmount();
        }
        uint256 lvlUSDBalBefore = lvlUSD.balanceOf(address(this));
        _mintlvlUSD(token, collateralAmount);
        uint256 lvlUSDBalAfter = lvlUSD.balanceOf(address(this));
    ...
    {
        require(
            _maxSlippageThresholdBasisPoints <= MAX_BASIS_POINTS, "Slippage threshold cannot exceed max basis points"
        );
