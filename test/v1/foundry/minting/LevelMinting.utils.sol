...
contract LevelMintingUtils is MintingBaseSetup {
    function maxMint_perBlock_exceeded_revert(uint256 excessiveMintAmount) public {
        ...
        assertEq(lvlusdToken.balanceOf(beneficiary), 0, "The beneficiary balance should be 0");
        assertEq(DAIToken.balanceOf(address(LevelMintingContract)), 0, "The level minting DAI balance should be 0");
        assertEq(DAIToken.balanceOf(benefactor), _DAIToDeposit, "Mismatch in DAI balance");
    }
    ...
    function executeRedeem() public {
        ILevelMinting.Order memory redeemOrder = redeem_setup(_lvlusdToMint, _DAIToDeposit, false, address(DAIToken));
        vm.prank(redeemer);
        LevelMintingContract.__redeem(redeemOrder);
    }
}
