...
import {IPool} from "@level/src/v1/interfaces/aave/IPool.sol";
import {IlvlUSD} from "@level/src/v1/interfaces/IlvlUSD.sol";
import {StakedlvlUSD} from "@level/src/v1/StakedlvlUSD.sol";
import {AaveV3YieldManager} from "@level/src/v1/yield/AaveV3YieldManager.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

import {LevelBaseReserveManager} from "@level/src/v1/reserve/LevelBaseReserveManager.sol";
import {EigenlayerReserveManager} from "@level/src/v1/reserve/LevelEigenlayerReserveManager.sol";
import {WrappedRebasingERC20} from "@level/src/v1/WrappedRebasingERC20.sol";
import {ILevelMinting} from "@level/src/v1/interfaces/ILevelMinting.sol";
import {ILevelMintingEvents} from "@level/src/v1/interfaces/ILevelMintingEvents.sol";
import {LevelMintingChild} from "./LevelMintingChild.sol";
import {ISingleAdminAccessControl} from "@level/src/v1/interfaces/ISingleAdminAccessControl.sol";
import {IlvlUSDDefinitions} from "@level/src/v1/interfaces/IlvlUSDDefinitions.sol";
import {ILevelBaseReserveManager} from "@level/src/v1/interfaces/ILevelBaseReserveManager.sol";
...
contract MintingBaseSetup is Test, ILevelMintingEvents, IlvlUSDDefinitions {
    ...
    // error encodings
    bytes internal InvalidCooldown = abi.encodeWithSelector(ILevelMinting.InvalidCooldown.selector);
    bytes internal MinimumCollateralAmountNotMet =
        abi.encodeWithSelector(ILevelMinting.MinimumCollateralAmountNotMet.selector);

    bytes internal MinimumlvlUSDAmountNotMet = abi.encodeWithSelector(ILevelMinting.MinimumlvlUSDAmountNotMet.selector);
    bytes internal Duplicate = abi.encodeWithSelector(ILevelMinting.Duplicate.selector);
    bytes internal InvalidAddress = abi.encodeWithSelector(ILevelMinting.InvalidAddress.selector);
    bytes internal InvalidlvlUSDAddress = abi.encodeWithSelector(ILevelMinting.InvalidlvlUSDAddress.selector);
    bytes internal InvalidAssetAddress = abi.encodeWithSelector(ILevelMinting.InvalidAssetAddress.selector);
    bytes internal InvalidOrder = abi.encodeWithSelector(ILevelMinting.InvalidOrder.selector);
    bytes internal InvalidAffirmedAmount = abi.encodeWithSelector(ILevelMinting.InvalidAffirmedAmount.selector);
    bytes internal InvalidAmount = abi.encodeWithSelector(ILevelMinting.InvalidAmount.selector);
    bytes internal InvalidRoute = abi.encodeWithSelector(ILevelMinting.InvalidRoute.selector);
    bytes internal InvalidAdminChange = abi.encodeWithSelector(ISingleAdminAccessControl.InvalidAdminChange.selector);
    bytes internal UnsupportedAsset = abi.encodeWithSelector(ILevelMinting.UnsupportedAsset.selector);
    bytes internal NoAssetsProvided = abi.encodeWithSelector(ILevelMinting.NoAssetsProvided.selector);
    bytes internal InvalidNonce = abi.encodeWithSelector(ILevelMinting.InvalidNonce.selector);
    bytes internal MaxMintPerBlockExceeded = abi.encodeWithSelector(ILevelMinting.MaxMintPerBlockExceeded.selector);
    bytes internal MaxRedeemPerBlockExceeded = abi.encodeWithSelector(ILevelMinting.MaxRedeemPerBlockExceeded.selector);
    // lvlUSD error encodings
    bytes internal OnlyMinterErr = abi.encodeWithSelector(IlvlUSDDefinitions.OnlyMinter.selector);
    bytes internal ZeroAddressExceptionErr = abi.encodeWithSelector(IlvlUSDDefinitions.ZeroAddressException.selector);
    bytes internal OperationNotAllowedErr = abi.encodeWithSelector(IlvlUSDDefinitions.OperationNotAllowed.selector);
    bytes internal IsOwnerErr = abi.encodeWithSelector(IlvlUSDDefinitions.IsOwner.selector);
    bytes internal DenylistedErr = abi.encodeWithSelector(IlvlUSDDefinitions.Denylisted.selector);
    ...
    function setUp() public virtual {
        ...
        stakedlvlUSD = new StakedlvlUSD(lvlusdToken, address(owner), address(owner));
        ...
        aaveYieldManager = new AaveV3YieldManager(IPool(mockAavePool), address(owner));
        // create ERC20 wrapper for aToken
        aUSDC = MockAToken(mockAavePool.getReserveData(address(USDCToken)).aTokenAddress);
        address aDAIToken = mockAavePool.getReserveData(address(DAIToken)).aTokenAddress;
        waUSDC = new WrappedRebasingERC20(IERC20(address(aUSDC)), "waUSDC", "waUSDC");
        waDAIToken = new WrappedRebasingERC20(IERC20(aDAIToken), "waDAI", "waDAI");
    ...
    function _generateRouteTypeHash(ILevelMinting.Route memory route) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
                ROUTE_TYPE, keccak256(abi.encodePacked(route.addresses)), keccak256(abi.encodePacked(route.ratios))
            )
        );
    }

    function signOrder(uint256 key, bytes32 digest, ILevelMinting.SignatureType sigType)
        public
        pure
        returns (ILevelMinting.Signature memory)
    {
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(key, digest);
        bytes memory sigBytes = _packRsv(r, s, v);

        ILevelMinting.Signature memory signature =
            ILevelMinting.Signature({signature_type: sigType, signature_bytes: sigBytes});

        return signature;
    }

    // Generic mint setup reused in the tests to reduce lines of code
    function mint_setup(uint256 lvlusdAmount, uint256 collateralAmount, bool multipleMints, address collateral)
        public
        returns (ILevelMinting.Order memory order, ILevelMinting.Route memory route)
    {
        ...
        IERC20(collateral).approve(address(LevelMintingContract), collateralAmount);
        vm.stopPrank();
    ...
    function redeem_setup(uint256 lvlusdAmount, uint256 collateralAmount, bool multipleRedeem, address collateral)
        public
        returns (ILevelMinting.Order memory redeemOrder)
    {
        (ILevelMinting.Order memory mintOrder, ILevelMinting.Route memory route) =
    ...
    function _getInvalidRoleError(bytes32 role, address account) internal pure returns (bytes memory) {
