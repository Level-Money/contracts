...
contract USDT is IERC20 {
    ...
    event DebugTransfer(address indexed from, address indexed to, uint256 value, bool isContract, bool callbackSuccess);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        try IMaliciousContract(recipient).onERC20Transfer(sender, amount) returns (bool success) {
        ...
        emit DebugTransfer(sender, recipient, amount, isContract, callbackSuccess);
    ...
    function _safeTransfer(address recipient, uint256 amount) internal {
        ...
        if (recipient.code.length > 0) {
            try IERC20Receiver(recipient).onERC20Received(msg.sender, msg.sender, amount, "") returns (bytes4 retval) {
                require(retval == IERC20Receiver.onERC20Received.selector, "SafeERC20: ERC20 receiving failed");
            } catch (bytes memory reason) {
                if (reason.length == 0) {
                    revert("SafeERC20: transfer to non-ERC20Receiver implementer");
                } else {
...
interface IMaliciousContract {
    function onERC20Transfer(address sender, uint256 amount) external returns (bool);
}
...
contract MaliciousContract is IMaliciousContract {
    ...
    function onERC20Transfer(address sender, uint256 amount) external override returns (bool) {
...
contract LevelMintingReentrancyTest is LevelMintingUtils {
    ...
    function setUp() public override {
        ...
        maliciousContract =
            new MaliciousContract(LevelMintingContract, IERC20(address(lvlusdToken)), IERC20(address(usdt)));

        // Grant necessary roles to the malicious contract
        vm.startPrank(owner);
        LevelMintingContract.grantRole(redeemerRole, address(maliciousContract));
        vm.stopPrank();
    ...
    function testReentrancyAttack() public {
        ...
        uint256 initialLvlUSDBalance = lvlusdToken.balanceOf(address(maliciousContract));
        ...
        uint256 finalLvlUSDBalance = lvlusdToken.balanceOf(address(maliciousContract));
        ...
        console.log("Reentrancy attempted:", maliciousContract.reentrancyAttempted());

        // Verify that a reentrancy attempt was made
        assertTrue(maliciousContract.reentrancyAttempted(), "No reentrancy attempt was made");

        // If the contract is secure, only one redeem should have succeeded
        assertEq(
            finalLvlUSDBalance, initialLvlUSDBalance - attackAmount, "lvlUSD balance should decrease by attack amount"
        );
        assertEq(finalUsdtBalance, initialUsdtBalance + attackAmount, "usdt balance should increase by attack amount");
        ...
        assertLt(finalUsdtBalance, initialUsdtBalance + attackAmount * 2, "Reentrancy attack seems to have succeeded");
