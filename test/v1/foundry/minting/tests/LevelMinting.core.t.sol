...
contract LevelMintingCoreTest is LevelMintingUtils {
    ...
    function test_redeem() public {
        executeRedeem();
        assertEq(DAIToken.balanceOf(address(LevelMintingContract)), 0, "Mismatch in DAI balance");
        assertEq(DAIToken.balanceOf(beneficiary), _DAIToDeposit, "Mismatch in DAI balance");
        assertEq(lvlusdToken.balanceOf(beneficiary), 0, "Mismatch in lvlUSD balance");
    }

    function test_initiate_and_complete_redeem() public {
        vm.prank(owner);
        LevelMintingContract.setMaxRedeemPerBlock(type(uint256).max);
        ILevelMinting.Order memory redeemOrder = redeem_setup(50 wei, 50 wei, false, address(DAIToken));
        vm.prank(owner);
        LevelMintingContract.grantRole(redeemerRole, beneficiary);
        vm.stopPrank();

        (, ILevelMinting.Route memory route) = mint_setup(500 wei, 500 wei, false, address(DAIToken));
        ...
        vm.warp(10 days);
        uint256 bal = DAIToken.balanceOf(beneficiary);
        LevelMintingContract.completeRedeem(redeemOrder.collateral_asset);
        uint256 new_val = DAIToken.balanceOf(beneficiary);
    ...
    function test_fuzz_initiate_and_complete_redeem(
        ...
        ILevelMinting.Order memory redeemOrder = redeem_setup(lvlusdAmount, collateralAmount, false, address(DAIToken));

        vm.prank(owner);
        LevelMintingContract.grantRole(redeemerRole, beneficiary);
        vm.stopPrank();

        (, ILevelMinting.Route memory route) = mint_setup(lvlusdAmount, collateralAmount, false, address(DAIToken));
    ...
    function test_initiate_and_complete_redeem_stablecoin_depeg_price_above_unit() public {
        vm.startPrank(owner);
        ...
        LevelMintingContract.setMaxRedeemPerBlock(type(uint256).max);
        ILevelMinting.Order memory redeemOrder = redeem_setup(50 wei, 40 wei, false, address(DAIToken));
        vm.prank(owner);
        LevelMintingContract.grantRole(redeemerRole, beneficiary);
        vm.stopPrank();
    ...
    function test_initiate_and_complete_redeem_stablecoin_depeg_price_below_unit() public {
        vm.startPrank(owner);
    ...
    function test_initiate_and_complete_redeem_min_collateral_not_met_revert() public {
        vm.prank(owner);
        LevelMintingContract.setMaxRedeemPerBlock(type(uint256).max);
        ILevelMinting.Order memory redeemOrder = redeem_setup(50 wei, 51 wei, false, address(DAIToken));
        vm.prank(owner);
        LevelMintingContract.grantRole(redeemerRole, beneficiary);
        vm.stopPrank();
        (, ILevelMinting.Route memory route) = mint_setup(500 wei, 500 wei, false, address(DAIToken));
    ...
    function test__usdc__mintMinLvlUSDAmountNotMetRevert(
        uint256 mintDollarAmount,
        uint256 lvlUSDDollarAmount,
        int256 mockOraclePriceDeviationBps
    ) public {
        ...
        vm.assume(lvlUSDDollarAmount > (mintDollarAmount * uint256(1e4 + mockOraclePriceDeviationBps)) / 1e4);
        mockOracle.updatePriceAndDecimals(
            int256((1e8 * (1e4 + mockOraclePriceDeviationBps)) / 1e4), mockOracle.decimals()
        );
        _test_mint_min_lvlusd_amount_not_met_revert(
            USDCToken, address(mockOracle), mintDollarAmount, lvlUSDDollarAmount
        );
    }
    ...
    function test__dai__mintMinLvlUSDAmountNotMetRevert(
        ...
        _test_mint_min_lvlusd_amount_not_met_revert(DAIToken, address(mockOracle), mintDollarAmount, lvlUSDDollarAmount);
    }
    ...
    function test__usdc__mintLvlUSD(
        ...
        uint256 newOraclePrice = uint256((1e8 * (1e4 + mockOraclePriceDeviationBps)) / 1e4);
        // we multiple the LHS and RHS by 1e18 to make sure that the statement is not true because of rounding errors
        vm.assume((1e18 * mintDollarAmount * newOraclePrice) / 1e8 >= 1e18 * lvlUSDDollarAmount);
        vm.assume(lvlUSDDollarAmount > 0);
        mockOracle.updatePriceAndDecimals(int256(newOraclePrice), mockOracle.decimals());
        _test_mint_lvlusd(USDCToken, address(mockOracle), mintDollarAmount, lvlUSDDollarAmount);
    }
    ...
    function test__dai__mintLvlUSD(
        ...
        vm.assume(lvlUSDDollarAmount > 0);
        mockOracle.updatePriceAndDecimals(int256(newOraclePrice), mockOracle.decimals());
        _test_mint_lvlusd(DAIToken, address(mockOracle), mintDollarAmount, lvlUSDDollarAmount);
    }

    function _normalizeAmount(uint256 amount, address token) public returns (uint256) {
    ...
    function _test_mint_min_lvlusd_amount_not_met_revert(
        ...
        LevelMintingContract.grantRole(redeemerRole, beneficiary);
        vm.stopPrank();
        (, ILevelMinting.Route memory route) = mint_setup(50 ether, 50 ether, false, address(token));
        ILevelMinting.Order memory order = ILevelMinting.Order({
            order_type: ILevelMinting.OrderType.MINT,
            benefactor: beneficiary,
            beneficiary: beneficiary,
            collateral_asset: address(token),
            lvlusd_amount: _normalizeAmount(lvlUSDDollarAmount, address(lvlusdToken)),
            collateral_amount: _normalizeAmount(mintDollarAmount, address(token))
        });
    ...
    function _test_mint_lvlusd(MockToken token, address oracle, uint256 mintDollarAmount, uint256 lvlUSDDollarAmount)
        public
    {
        vm.startPrank(owner);
        LevelMintingContract.addOracle(address(token), oracle);
        ...
        LevelMintingContract.grantRole(redeemerRole, beneficiary);
        vm.stopPrank();
        (, ILevelMinting.Route memory route) = mint_setup(500000000 ether, 500000000 ether, false, address(token));
        ILevelMinting.Order memory order = ILevelMinting.Order({
            order_type: ILevelMinting.OrderType.MINT,
            benefactor: beneficiary,
            beneficiary: beneficiary,
        ...
        uint256 beneficiaryBalBefore = ERC20(token).balanceOf(beneficiary);
        uint256 reserveBalBefore = ERC20(token).balanceOf(address(LevelMintingContract));
        uint256 lvlusdTokenSupplyBefore = lvlusdToken.totalSupply();
        LevelMintingContract.mint(order, route);
        uint256 reserveBalAfter = ERC20(token).balanceOf(address(LevelMintingContract));
        assertEq(reserveBalAfter - reserveBalBefore, _normalizeAmount(mintDollarAmount, address(token)));
        assertEq(
        ...
        _checkSolvency(address(token), address(LevelMintingContract), oracle, true);
        vm.stopPrank();
    }
    ...
    function _checkSolvency(
        ...
        uint256 reserveBal = ERC20(collateral).balanceOf(address(reserve));
        // calculate the reserve value scaled up by the difference in decimals
        // between lvlUSD and collateral asset
        uint256 scaledReserveValue =
            (10 ** (18 - collateralDecimals) * reserveBal * uint256(_price)) / (10 ** MockOracle(oracle).decimals());
        if (checkEquality) {
    ...
    function test_initiate_and_complete_redeem_insufficient_cooldown_revert() public {
        vm.prank(owner);
        LevelMintingContract.setMaxRedeemPerBlock(type(uint256).max);
        ILevelMinting.Order memory redeemOrder = redeem_setup(50 wei, 50 wei, false, address(DAIToken));
        (, ILevelMinting.Route memory route) = mint_setup(50 wei, 50 wei, false, address(DAIToken));
        ILevelMinting.Order memory order = ILevelMinting.Order({
            order_type: ILevelMinting.OrderType.MINT,
            benefactor: beneficiary,
            beneficiary: beneficiary,
    ...
    function test_dai_withdraw() public {
        ...
        ILevelMinting.Route memory route = ILevelMinting.Route({addresses: targets, ratios: ratios});
    ...
    function test_fuzz_mint_noSlippage(uint256 expectedAmount) public {
        ...
        (ILevelMinting.Order memory order, ILevelMinting.Route memory route) =
        ...
        assertEq(DAIToken.balanceOf(address(LevelMintingContract)), _DAIToDeposit);
    ...
    function test_multipleValid_reserveRatios_addresses() public {
        ...
        assertEq(DAIToken.balanceOf(address(reserve1)), (_DAIToDeposit * 4) / 10);
        assertEq(DAIToken.balanceOf(address(reserve2)), (_DAIToDeposit * 3) / 10);
        assertEq(DAIToken.balanceOf(address(LevelMintingContract)), (_DAIToDeposit * 3) / 10);
    ...
    function test_fuzz_multipleInvalid_reserveRatios_revert(uint256 ratio1) public {
    ...
    function test_fuzz_singleInvalid_reserveRatio_revert(uint256 ratio1) public {
    ...
    function test_sending_redeem_order_to_mint_revert() public {
        ILevelMinting.Order memory order = redeem_setup(1 ether, 50 ether, false, address(DAIToken));
    ...
    function test_mismatchedAddressesAndRatios_revert() public {
        uint256 _smallLvlUsdToMint = 1.75 * 10 ** 23;
        (ILevelMinting.Order memory order, ILevelMinting.Route memory route) =
            mint_setup(_smallLvlUsdToMint, _DAIToDeposit, false, address(DAIToken));
