...
contract LevelMintingBlockLimitsTest is LevelMintingUtils {
    ...
    function test_multiple_mints() public {
        ...
        (ILevelMinting.Order memory aOrder, ILevelMinting.Route memory aRoute) =
            mint_setup(firstMintAmount, _DAIToDeposit, false, address(DAIToken));
        ...
        (ILevelMinting.Order memory bOrder, ILevelMinting.Route memory bRoute) =
            mint_setup(secondMintAmount, _DAIToDeposit, true, address(DAIToken));
        vm.prank(minter);
        ...
        assertTrue(
            LevelMintingContract.mintedPerBlock(block.number) < maxMintAmount, "Mint amount exceeded without revert"
        );
    }
    ...
    function test_fuzz_mint_maxMint_perBlock_exceeded_revert(uint256 excessiveMintAmount) public {
        ...
        (ILevelMinting.Order memory mintOrder, ILevelMinting.Route memory route) =
            mint_setup(excessiveMintAmount, _DAIToDeposit, false, address(DAIToken));
        ...
        assertEq(lvlusdToken.balanceOf(beneficiary), 0, "The beneficiary lvlUSD balance should be 0");
    }

    function test_fuzz_nextBlock_mint_is_zero(uint256 mintAmount) public {
        vm.assume(mintAmount < LevelMintingContract.maxMintPerBlock() && mintAmount > 0);
    ...
    function test_fuzz_maxMint_perBlock_setter(uint256 newMaxMintPerBlock) public {
        ...
        assertEq(LevelMintingContract.maxMintPerBlock(), newMaxMintPerBlock, "The max mint per block setter failed");
    }
    ...
    function test_multiple_redeem() public {
        ...
        ILevelMinting.Order memory redeemOrder =
            redeem_setup(firstRedeemAmount, _DAIToDeposit, false, address(DAIToken));
        ...
        ILevelMinting.Order memory bRedeemOrder =
            redeem_setup(secondRedeemAmount, _DAIToDeposit, true, address(DAIToken));
        ...
        assertTrue(
            LevelMintingContract.redeemedPerBlock(block.number) < maxRedeemAmount,
    ...
    function test_fuzz_maxRedeem_perBlock_exceeded_revert(uint256 excessiveRedeemAmount) public {
        ...
        vm.assume(excessiveRedeemAmount > LevelMintingContract.maxRedeemPerBlock());
        ...
        ILevelMinting.Order memory redeemOrder =
            redeem_setup(excessiveRedeemAmount, _DAIToDeposit, false, address(DAIToken));

        vm.startPrank(redeemer);
        vm.expectRevert(MaxRedeemPerBlockExceeded);
        LevelMintingContract.__redeem(redeemOrder);

        assertEq(DAIToken.balanceOf(address(LevelMintingContract)), _DAIToDeposit, "Mismatch in DAI balance");
        assertEq(DAIToken.balanceOf(beneficiary), 0, "Mismatch in DAI balance");
        assertEq(lvlusdToken.balanceOf(beneficiary), excessiveRedeemAmount, "Mismatch in lvlUSD balance");

        vm.stopPrank();
    }

    function test_fuzz_nextBlock_redeem_is_zero(uint256 redeemAmount) public {
        vm.assume(redeemAmount < LevelMintingContract.maxRedeemPerBlock() && redeemAmount > 0);
        ILevelMinting.Order memory redeemOrder = redeem_setup(redeemAmount, _DAIToDeposit, false, address(DAIToken));
    ...
    function test_fuzz_maxRedeem_perBlock_setter(uint256 newMaxRedeemPerBlock) public {
        ...
        assertEq(
            LevelMintingContract.maxRedeemPerBlock(), newMaxRedeemPerBlock, "The max redeem per block setter failed"
        );
    }
}
