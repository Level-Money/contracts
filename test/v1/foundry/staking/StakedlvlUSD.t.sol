...
import "@level/src/v1/interfaces/IERC20Events.sol";
import "@level/src/v1/interfaces/IStakedlvlUSDCooldown.sol";
...
contract StakedlvlUSDTest is Test, IERC20Events {
    ...
    bytes32 WITHDRAW_FROM_FREEZER_ROLE = keccak256("WITHDRAW_FROM_FREEZER_ROLE");
    ...
    function setUp() public virtual {
        ...
        vm.prank(owner);
        stakedlvlUSD = new StakedlvlUSD(IERC20v4(address(lvlUSDToken)), rewarder, owner);
    ...
    function _transferRewards(uint256 amount, uint256 expectedNewVestingAmount) internal {
        ...
        assertApproxEqAbs(stakedlvlUSD.getUnvestedAmount(), expectedNewVestingAmount, 1);
        vm.stopPrank();
    }
    ...
    function testlvlUSDValuePerStlvlUSD() public {
        ...
        assertApproxEqAbs(stakedlvlUSD.convertToAssets(1 ether), (vestedAmount * 1 ether) / 150 ether, 1);
        ...
        assertApproxEqAbs(lvlUSDToken.balanceOf(alice), (vestedAmount * 2) / 3, 2);
    ...
    function testFuzzFairStakeAndUnstakePrices(
        ...
        uint256 bobStakedlvlUSD = (amount2 * (amount1 + 1)) / (vestedAmount + 1);
        ...
        uint256 gregStakedlvlUSD = (amount3 * (stakedlvlUSD.totalSupply() + 1)) / (vestedAmount + 1);
        ...
        uint256 lvlusdPerStakedlvlUSDBefore = stakedlvlUSD.convertToAssets(1 ether);
        uint256 bobUnstakeAmount = (stakedlvlUSD.balanceOf(bob) * (vestedAmount + 1)) / (stakedlvlUSD.totalSupply() + 1);
        uint256 gregUnstakeAmount =
            (stakedlvlUSD.balanceOf(greg) * (vestedAmount + 1)) / (stakedlvlUSD.totalSupply() + 1);

        if (bobUnstakeAmount > 0) _redeem(bob, stakedlvlUSD.balanceOf(bob));
        uint256 lvlusdPerStakedlvlUSDAfter = stakedlvlUSD.convertToAssets(1 ether);
        if (lvlusdPerStakedlvlUSDAfter != 0) {
            assertApproxEqAbs(lvlusdPerStakedlvlUSDAfter, lvlusdPerStakedlvlUSDBefore, 1 ether);
        }
    ...
    function testDonationAttack() public {
        ...
        assertEq(stakedlvlUSD.totalSupply(), initialStake);
        assertEq(lvlUSDToken.balanceOf(address(stakedlvlUSD)), initialStake + donationAmount);
        _mintApproveDeposit(bob, bobStake);
        uint256 bobStlvlUSDBal = stakedlvlUSD.balanceOf(bob);
        uint256 bobStlvlUSDExpectedBal = (bobStake * initialStake) / (initialStake + donationAmount);
