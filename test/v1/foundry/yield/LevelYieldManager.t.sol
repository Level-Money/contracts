...
contract LevelYieldManagerTest is MintingBaseSetup {
    ...
    function testDepositForYieldAndWithdraw() public {
        ...
        assertEq(waUSDC.balanceOf(address(owner)), 10, "Incorrect waUSDC balance.");
        waUSDC.approve(address(aaveYieldManager), amount);
        aaveYieldManager.withdraw(address(USDCToken), 10);
        assertEq(waUSDC.balanceOf(address(owner)), 0, "Incorrect waUSDC balance.");
        assertEq(USDCToken.balanceOf(address(owner)), amount, "Incorrect USDC balance.");
    }
    ...
    function testDepositForYieldAndWithdrawMoreDecimals() public {
        ...
        assertEq(waDAIToken.balanceOf(address(owner)), 10, "Incorrect waDAIToken balance.");
        waDAIToken.approve(address(aaveYieldManager), amount);
        aaveYieldManager.withdraw(address(DAIToken), 10);
        assertEq(waDAIToken.balanceOf(address(owner)), 0, "Incorrect waDAIToken balance.");
        assertEq(DAIToken.balanceOf(address(owner)), amount, "Incorrect DAIToken balance.");
    }
    ...
    function testReserveManagerDepositForYield() public {
        vm.startPrank(owner);
        uint256 amount = 10000000000000;
        uint256 transferAmount = 99999999;
        USDCToken.mint(amount);
        USDCToken.transfer(address(eigenlayerReserveManager), transferAmount);
        eigenlayerReserveManager.approveSpender(address(waUSDC), address(aaveYieldManager), amount);

        vm.stopPrank();
        vm.startPrank(managerAgent);
        eigenlayerReserveManager.depositForYield(address(USDCToken), 10);
        assertEq(waUSDC.balanceOf(address(eigenlayerReserveManager)), 10, "Incorrect waUSDC balance.");
        eigenlayerReserveManager.withdrawFromYieldManager(address(USDCToken), 10);
        assertEq(waUSDC.balanceOf(address(eigenlayerReserveManager)), 0, "Incorrect waUSDC balance.");
        assertEq(USDCToken.balanceOf(address(eigenlayerReserveManager)), transferAmount, "Incorrect USDC balance.");
    }
    ...
    function testAaveAccrueInterest(uint256 depositAmount, uint256 increaseAmountPercent) public {
        ...
        vm.startPrank(managerAgent);
        eigenlayerReserveManager.depositForYield(address(USDCToken), depositAmount);
        vm.stopPrank();
        assertEq(waUSDC.balanceOf(address(eigenlayerReserveManager)), depositAmount, "Incorrect waUSDC balance.");
        assertApproxEqRel(aUSDC.balanceOf(address(waUSDC)), depositAmount, 1e15);
        ...
        assertApproxEqRel(aUSDC.balanceOf(address(waUSDC)), newTotalAmountQuotedInUnderlying, 1e15);
        waUSDC.grantRole(waUSDC.RECOVERER_ROLE(), bob);
        vm.stopPrank();
        vm.startPrank(bob);

        // recover accrued interest in the form of the underlying asset
        uint256 value = waUSDC.recoverUnderlying();
